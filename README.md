...
# Leverage Long Position Monorepo
...
```
npx degit wighawag/leverage-long-position-monorepo <your-app-folder>
```
...
```
npx degit wighawag/leverage-long-position-monorepo#nft <your-app-folder>
```
-->
...
this will update name of the project (by default "leverage-long-position-monorepo") to be the name of the folder (See `set-name` command) and install the dependencies (`pnpm install`)
<br/><br/>

`pnpm set-name [<new name>]`

This will replace every instance of `leverage-long-position-monorepo` (or whatever name was set) to `new name` (if specified, otherwise it use the folder name)
